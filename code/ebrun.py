...
def run_ebscore(
    ...
    # ############## LOAD DATA ###############################
    show_output(
        f"Computing EBscore for chrom {chrom} on target {bam_file}", color="normal"
    )

    ############### bam2matrix #######################
    show_output(f"Piling up {chrom} of {bam_file} with Pon List.", color="normal")
    ...
    if df.empty:
        show_output(f"Pileup for {chrom} of {bam_file} was empty!")
        if debug:
    ...
    # ############## matrix2AB ######################
    show_output(f"Computing ABparams from PoN data on {chrom}.", color="normal")
    df["ABparams"] = df.apply(matrix2AB, axis=1)
    ...
    show_output(
        f"Created EBscore for chrom {chrom} of {bam_file} and written to {output_file}",
        color="success",
    )
    return EB_df


def run_eb_from_cache(
    table,
    tumor_bam,
    output,
    pon_list,
    ...
    # ############## LOAD DATA ###############################
    show_output(
        f"Computing EBscore for chrom {chrom} of {tumor_bam} using EBcache {AB_cache_file}",
        color="normal",
    )
    ...
    if mut_df.empty:
        ...
        show_output(
            f"No mutations for {chrom} in mutation list! Writing empty file to {output[0]}",
            color="warning",
        )
    else:
