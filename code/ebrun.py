...
def run_ebscore(
    ...
    matrix_df = tumor2matrix(mut_file,
                                bam=tumor_bam,
                                pileup=pileup_file,
                                chrom=chrom, 
                                pon_list=pon_list, 
    ...
    if matrix_df.empty:
        ...
        if debug:
            show_output(f"Created empty file {matrix_file}", color="warning", time=False)
        return
    ...
    if "AB" in matrix_df.columns:
        show_output(
            f"PONmatrix and AB computation on chrom {chrom} of {tumor_file} finished. Used ABcache!", color="success"
        )
        AB_df = matrix_df
    else:
        show_output(
            f"PONmatrix computation on chrom {chrom} of {tumor_file} finished. Starting AB computation..", color="success"
        )
